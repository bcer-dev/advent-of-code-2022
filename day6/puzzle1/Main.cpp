...
{
	EXPECT_EQ(findPacketMarker("bvwbjplbgvbhsrlpgdmjqwftvncz"), 5);
}

TEST(PacketMarkerTests, HandlesSample2)
{
	EXPECT_EQ(findPacketMarker("nppdvjthqldpwncqszvftbrmjlhg"), 6);
}

TEST(PacketMarkerTests, HandlesSample3)
{
	EXPECT_EQ(findPacketMarker("nznrnfrfntjfmvfwmzdfjlvtqnbhcprsg"), 10);
}

TEST(PacketMarkerTests, HandlesSample4)
{
	EXPECT_EQ(findPacketMarker("zcfzfwzzqfrljwzlrfnpqdbhtmscgvjw"), 11);
}
